...
| 5. Model quantization | **Real measurement, TinyLlama-1.1B (2026-08-22)** | Memory: ✅ 4.82× smaller than fp16 (PASS <25% target, both schemes). Accuracy: ❌ perplexity 12.8 → 89,101 (per-tensor) → **132,590 (per-channel — worse, not better)**. Naive post-training ternary quantization catastrophically breaks the model either way; finer-grained scaling isn't the fix. See note below |
...
**Phase 4 GEMM kernel fix (2026-08-20, register-blocking tuned 2026-08-22):** investigation found the CSC/CSR "zero-skip" kernel wasn't naive (already AVX2 + OpenMP + cache-aware) but had two structural problems: it vectorizes over the batch dimension while Phase 4 tests batch=1 (so AVX2 never engaged), and at ternary's real ~33-40% zero density, CSC/CSR index storage is ~3.3× *larger* than just the dense int8 weight array — "zero-skip" was optimizing the wrong resource on a memory-bandwidth-bound kernel. A new dense-packed kernel (`src/core/simd/ternary_gemm_dense.h`, `DenseWeights` in `ternary_zero_skip_gemm`) fixes both: measured **13×-41× faster than the best existing kernel at batch=1**, native and pybind-free (`benchmarks/cpp-native-kernels/bench_gemm_dense.cpp`). Fixing this also surfaced a benchmark-methodology bug (a 3-call warmup that was fine for the old ~1-5ms/call kernel left this machine's CPU frequency-scaling cold for the new ~10-30µs/call one, causing up to 50× run-to-run variance) — fixed with wall-clock warmup and interleaved median timing; stable across repeated runs. The kernel's one honest weak spot at launch — losing 0.45× to the old kernel at batch=128 for the smallest shape — was closed 2026-08-22 by sweeping the register-blocking factor (`MB=4`→`8`): now **every** (shape × batch) cell the suite measures wins, 1.14×-41×. Full investigation: [reports/2026-08-20/GEMM_DENSE_PACKED_OPTIMIZATION.md](reports/2026-08-20/GEMM_DENSE_PACKED_OPTIMIZATION.md).

**Phase 5 model quantization, real result (2026-08-22):** `benchmarks/model_quantization/quantize_tinyllama.py` quantizes every attention/MLP weight in TinyLlama-1.1B-Chat-v1.0 to ternary (BitNet-style absmean scale — exactly this project's own documented "simple threshold-based" scheme, no fine-tuning/QAT/calibration) and measures WikiText-2 perplexity before/after. **Per-tensor** (one scale for the whole weight matrix): memory genuinely drops 4.82× (2200→456MB for the quantized layers, passes the <25%-of-fp16 target), but perplexity explodes from 12.78 to 89,101 (both cross-entropy values finite and sanity-checked, not a NaN/Inf bug) — a decisive failure of the accuracy criterion. **Same-day follow-up, per-channel** (one scale per output row — the obvious next-cheapest refinement): made it *worse*, not better — perplexity 132,590. This falsifies "the scale was too coarse" as the explanation and sharpens the real conclusion: naive post-training ternary quantization of an already-converged checkpoint is broken by the lack of calibration/retraining itself, not by scale granularity — exactly what the published literature on ternary/1.58-bit techniques (which train from scratch with QAT) would predict once examined. Zero fraction across quantized layers (~32-33% mean) matches this project's own established ~33-40% ternary sparsity figure both times — a real internal consistency check, not a coincidence. Full writeup: [reports/2026-08-22/PHASE5_TINYLLAMA_QUANTIZATION.md](reports/2026-08-22/PHASE5_TINYLLAMA_QUANTIZATION.md).
...
| Accuracy retention after quantization | < 5% loss | ❌ Tested 2026-08-22 (TinyLlama-1.1B, naive post-training quantization) — perplexity degraded +697,074% (per-tensor scale) and +1,037,361% (per-channel scale, tested same day — worse, not better); real QAT/fine-tuning or calibrated error-compensation (GPTQ/AWQ-style) would be needed for a fairer test, not attempted |
...
- **Profiler integration (VTune ITT, NVTX)** - the call-site framework exists in `src/core/profiling/ternary_profiler.h` and is genuinely wired into `bindings_core_ops.cpp`'s hot paths, but VTune needs Intel's proprietary tool and NVTX needs a CUDA GPU (neither available here) — both remain unbuilt. **Perfetto built, wired into the main module, and verified end-to-end 2026-08-25** — it's the one backend needing no special hardware/license; vendored the open-source SDK (`third_party/perfetto/`), wired a real backend into `ternary_profiler.h`, and produced this project's first real profiler trace via a native demo. Same day, wired into the actual `ternary_simd_engine` build too: `python build/build.py --enable-perfetto` (default builds unaffected) plus new `tc.perfetto_start()`/`tc.perfetto_stop()`/`tc.has_perfetto` Python bindings — verified with a real tracing session driven through actual `tc.tadd`/`tc.tmul` calls, checked against the resulting trace's own slice data (not just "it compiled"). Also fixed a real pre-existing latent bug found along the way: `build.py`'s `build_ext --inplace` could silently skip an entire rebuild (not just stale files) when flags changed without touching source files. See [reports/2026-08-25/PERFETTO_PROFILER_INTEGRATION.md](reports/2026-08-25/PERFETTO_PROFILER_INTEGRATION.md)
...
- **Real model quantization (Phase 5)** wired to real measurement code 2026-08-22 (TinyLlama-1.1B, naive post-training ternary quantization) — memory criterion passes (4.82× smaller than fp16, both schemes); accuracy criterion fails decisively for both per-tensor (12.8 → 89,101) and per-channel scaling (12.8 → 132,590, tested same day — worse, not better, ruling out "the scale is too coarse" as the fix). Real QAT/fine-tuning or calibrated error-compensation (GPTQ/AWQ-style) is the actual next step; extending to Phi-2/Gemma-2B would need further work either way, not attempted. **Power measurement (Phase 6)** wired to real code 2026-08-22 (auto-detects Intel RAPL / NVIDIA / Windows monitors, honest Mock fallback) but this project's dev sandbox has no reachable hardware monitor to actually validate against (no RAPL read permission, no `perf` access, no sudo) — needs running somewhere with real access before a citable number exists
...
- ✅ **Dense-packed GEMM kernel** - fixed the "0.189× avg / too slow for AI" matmul verdict; new kernel is 13×-41× faster than the old CSC/CSR one at the tested batch size, flips the competitive suite's Phase 4 verdict to "viable for AI". Its one launch-day weak spot (a loss at batch=128 for the smallest shape) closed 2026-08-22 by tuning register-blocking — every cell the suite measures now wins
...
- ✅ **Phase 5 real model quantization** - was purely descriptive; now runs a real TinyLlama-1.1B ternary quantization + WikiText-2 perplexity measurement, per-tensor and per-channel. Memory passes both ways (4.82× smaller than fp16); accuracy fails decisively both ways, and per-channel scaling makes it *worse* than per-tensor — falsifies "the scale is too coarse" and points at calibration/QAT as the real gap, not scale granularity
- ✅ **GEMM `MB=8` register-block tuning** - closed the one remaining loss in the dense-packed GEMM kernel (Small MLP at batch=128, was 0.45×); every (shape × batch) cell the native benchmark measures now wins, 1.14×-41×
- ✅ **Perfetto profiler integration** - this project's first real, verified profiler trace ever, now wired into the main `ternary_simd_engine` build (`--enable-perfetto`) with real Python bindings (`tc.perfetto_start`/`_stop`); VTune/NVTX remain blocked by hardware/license, but Perfetto needed neither and is now built and validated end-to-end
