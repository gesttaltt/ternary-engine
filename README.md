...
- **Profiler integration (VTune ITT, NVTX)** - the call-site framework exists in `src/core/profiling/ternary_profiler.h` and is genuinely wired into `bindings_core_ops.cpp`'s hot paths, but VTune needs Intel's proprietary tool and NVTX needs a CUDA GPU (neither available here) — both remain unbuilt. **Perfetto built and verified 2026-08-25** — it's the one backend needing no special hardware/license; vendored the open-source SDK (`third_party/perfetto/`), wired a real backend into `ternary_profiler.h`, and produced this project's first real profiler trace via a native demo (`build/build_perfetto_demo.py`), verified against the trace's own slice data (not just "it compiled"). Not yet wired into the main Python module build. See [reports/2026-08-25/PERFETTO_PROFILER_INTEGRATION.md](reports/2026-08-25/PERFETTO_PROFILER_INTEGRATION.md)
...
- ✅ **Perfetto profiler integration** - this project's first real, verified profiler trace ever; VTune/NVTX remain blocked by hardware/license, but Perfetto needed neither and is now built and validated end-to-end
