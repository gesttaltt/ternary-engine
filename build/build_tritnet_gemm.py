...
USAGE:
    python build/build_tritnet_gemm.py [options]
...
def build_module(naive_only=False, verbose=False, clean=False):
    ...
    if not naive_only:
        sources.append("models/tritnet/gemm/tritnet_gemm_avx2.cpp")
...
Examples:
  ...
  python build/build_tritnet_gemm.py --naive-only
      Build reference implementation only

  python build/build_tritnet_gemm.py --clean --verbose
      Clean rebuild with detailed output

  python build/build_tritnet_gemm.py --validate-only
