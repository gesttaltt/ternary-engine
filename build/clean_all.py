...
Usage:
    python build/clean_all.py                    # Clean all build artifacts
    python build/clean_all.py --keep-latest      # Keep latest builds
    python build/clean_all.py --keep-results 5   # Keep last 5 benchmark results
    python build/clean_all.py --dry-run          # Show what would be deleted
    python build/clean_all.py --deep             # Include external temp dirs
"""
...
class CleanupManager:
    ...
    def clean_deprecated_scripts(self):
        ...
        # Old location of build scripts (was in build/scripts/, then build/, now in build/)
