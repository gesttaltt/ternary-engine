...
8. **`BenchmarkRunner` unused** - `benchmarks/python-with-interpreter-overhead/benchmark_framework.py`'s `BenchmarkRunner` class is imported nowhere except itself and a deprecated file; `bench_fair_baseline.py`, `bench_simd_core_ops.py`, and `bench_simd_fusion_ops.py` each hand-roll their own warmup/measure/stats logic instead, with inconsistent statistical rigor between them (found 2026-08-12 while fixing a crash in `BenchmarkRunner` that none of the three active scripts would have hit, since none of them use it). **PARTIALLY RESOLVED 2026-08-18**: `bench_simd_fusion_ops.py` now calls `BenchmarkRunner` directly for its baseline-vs-fused measurement (an exact paradigm fit -- gains a CV-based stability check and a 95% CI it never had, at the cost of dropping min/max, which BenchmarkRunner doesn't track). `bench_fair_baseline.py` adopts the same statistics engine (`compute_timing_statistics()`, promoted out of `BenchmarkRunner._compute_statistics` to a standalone function) internally, while deliberately keeping its own `fair_baseline_*.json` schema byte-for-byte unchanged (verified: same top-level and cell keys before/after) -- that schema is a load-bearing dependency of `benchmark_validator.py`'s `extract_performance()`, itself already fixed once (2026-08-12/13) for exactly this schema, so silently changing it here would have reopened that bug. `bench_simd_core_ops.py` deliberately NOT changed: unlike the other two, it doesn't take repeated per-call timing samples at all -- it brackets one block of `MEASURED_ITERATIONS` back-to-back calls with a single timer and divides, which is the source of the "35,042 Mops/s peak throughput" headline figure this doc cites. Forcing BenchmarkRunner's repeated-single-call paradigm onto it would silently change what that headline number means without the revalidation-with-date-and-platform this doc's own "Modifying Hot Paths" section requires before touching a cited metric's methodology -- left as an open follow-up needing a deliberate, separately-validated session, not folded into this dedup pass. `tests/run_tests.py` 15/15 throughout; JSON output schemas of both changed scripts spot-checked directly (`fair_baseline_*.json` keys identical; `bench_simd_fusion_ops.py --output` still produces `speedup`/`separate_mean`/`fused_mean`).
...
**Version:** 1.36.0 · **Updated:** 2026-08-18 · **Project:** Ternary Engine · **Repository:** https://github.com/gesttaltt/ternary-engine
