...
**Doc-Type:** Project-Level Configuration · Version 1.10 · Updated 2026-08-14 · Author Ternary Engine Team
...
**Phase 3** - C++ Integration ⚠️ NAIVE BASELINE COMPLETE — TritNet loses badly, see caveat
- Export ternary weights to binary format ✅ `models/tritnet/inference/generate_weights_header.py` → `tritnet_weights.h` (constexpr, compile-time, matches project's LUT-codegen convention)
- Implement C++ inference engine ✅ `models/tritnet/inference/tritnet_inference.h` (naive/scalar, all 5 ops), verified bit-exact against every op's recorded checkpoint accuracy over the FULL input space (243/59,049 samples) in `tests/cpp/test_tritnet_inference.cpp`
- Benchmark TritNet vs LUT performance ✅ run 2026-08-14, Linux x64, AMD Ryzen 5 7520U, g++ 13.3.0 `-O3 -march=native` (`benchmarks/cpp-native-kernels/bench_tritnet_inference.cpp`): **LUT wins by 950×–1776× at the naive/scalar level** (tnot 265.1 vs 0.279 Mops/s; tadd 109.4 vs 0.072; tmul 126.7 vs 0.072; tmin 110.3 vs 0.072; tmax 129.0 vs 0.073 — all Mops/s of 5-trit-chunk ops). Matches the expected ~20K-more-MACs-per-op cost noted in research/PRIOR_ART_TERNARY_LANDSCAPE.md; not yet AVX2-vectorized. **Honest read**: closing a 3-orders-of-magnitude gap via SIMD alone is very unlikely — this project's own AVX2 kernels elsewhere in the repo report roughly 10–30× over scalar, nowhere near 1000×+. TritNet's practical case rests on Phase 4/5 (GPU/TPU throughput at batch scale, or learned generalization beyond what a LUT can express), not on beating a LUT at this per-op width on a CPU.
...
**Version:** 1.10.0 · **Updated:** 2026-08-14 · **Project:** Ternary Engine · **Repository:** https://github.com/gesttaltt/ternary-engine
