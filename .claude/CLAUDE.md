...
**Doc-Type:** Project-Level Configuration · Version 1.64 · Updated 2026-08-29 · Author Ternary Engine Team
...
- Power consumption: 2-4× better ⚠️ **Blocked by a file permission only —
  NOT by hardware** (corrected 2026-08-29; an earlier note here said "needs
  root or other hardware", which overstated it). RAPL works on this Ryzen;
  `/sys/class/powercap/intel-rapl:0/energy_uj` is simply mode 0400 root-only
  (the post-PLATYPUS mitigation). One `sudo chmod a+r` on this machine
  unblocks it — **the cheapest remaining criterion, one command from 3/5 →
  4/5.** GPU power is readable via `nvidia-smi` but measures the wrong
  device (the ternary kernels are CPU AVX2).
...
**Current status:** **3/5 criteria validated** (2026-08-29). Criterion 3 is
newly measured and passes in the low-batch regime that "latency" actually
denotes; criterion 2's earlier ⚠️ was stale bookkeeping rather than a new
result. The two open criteria are 4 (power — *blocked*, needs root or other
hardware, not engineering effort) and 5 (accuracy — *fails*, five techniques
measured, best is block-local QAT at +17,404%).
...
**Version:** 1.64.0 · **Updated:** 2026-08-29 · **Project:** Ternary Engine · **Repository:** https://github.com/gesttaltt/ternary-engine
