...
**Doc-Type:** Project-Level Configuration · Version 1.14 · Updated 2026-08-14 · Author Ternary Engine Team
...
- Benchmark TritNet vs LUT performance ✅ run 2026-08-14, Linux x64, AMD Ryzen 5 7520U, g++ 13.3.0 `-O3 -march=native -mavx2 -mfma` (`benchmarks/cpp-native-kernels/bench_tritnet_inference.cpp`): AVX2 gives a real **~10.2×–10.9× speedup over scalar** (tnot 2.75 vs 0.268 Mops/s; tadd 0.787 vs 0.072; tmul 0.783 vs 0.073; tmin 0.785 vs 0.073; tmax 0.782 vs 0.073 — landing at the low end of this repo's usual ~10–30× AVX2 gain, as predicted), but **LUT still wins by 169×–195×** even against the vectorized path (LUT throughput itself: tnot ~535 Mops/s, binary ops ~133 Mops/s — 5-trit-chunk ops/sec, consistent run-to-run). Matches the ~20K-more-MACs-per-op cost noted in research/PRIOR_ART_TERNARY_LANDSCAPE.md; AVX2 recovered roughly 1 order of magnitude, not the 3 needed to close the gap, exactly as anticipated. **Honest read confirmed, not just predicted**: TritNet's practical case rests on Phase 4/5 (GPU/TPU throughput at batch scale, or learned generalization beyond what a LUT can express), not on beating a LUT at this per-op width on a CPU — AVX2 does not change that conclusion, it was never going to. **Fairness check (2026-08-14, user-requested — same lesson this project already learned from the retired "8,234× vs Python" claim, see `core_innovation` above)**: the AVX2 kernel re-runs the int8→float weight conversion on every single forward-pass call even though the weights never change between calls — a fixed cost re-paid per-call instead of amortized. Isolated this by pre-converting weights once and reusing them across 200K calls, checked on **three** ops spanning both architectures in the model family and both checkpoint-accuracy extremes (correctness-verified bit-identical to the shipped AVX2 path before trusting any timing): tadd (hidden=128, 100% checkpoint) amortizes to a real **~1.82× speedup** (0.78 → 1.41 Mops/s), narrowing LUT's win from ~169–187× to **~93–103×**; tmul (hidden=128, 99.5% checkpoint — the weakest of the four binary ops) amortizes identically at **~1.82×** (0.78 → 1.43 Mops/s, ~187× → ~103×), confirming the ratio is a function of the shared architecture, not of how clean the learned weights are; tnot (hidden=64, the one unary op, different shape entirely) amortizes to **~1.72–1.74×** (2.72 → 4.72 Mops/s), narrowing its LUT win from ~195–197× to **~113–114×**. Consistent magnitude across every op checked, reproducible across runs, and it's the more representative number for anything that would actually batch (Phase 4's whole premise) — but it does not change the qualitative conclusion: LUT still wins by two orders of magnitude, not one.
...
**Version:** 1.14.0 · **Updated:** 2026-08-14 · **Project:** Ternary Engine · **Repository:** https://github.com/gesttaltt/ternary-engine
