...
3. **Competitive benchmarking** - Only 2/5 criteria validated. Partial Linux x64 rerun 2026-08-11 (interrupted, resume next session — see reports/2026-08-11/LINUX_VALIDATION_REPORT.md §3): Phase 1 (arithmetic) verdict "NEEDS WORK" (0.63-0.68x avg, matches fair-baseline finding that single ops are ~parity/behind NumPy); Phase 2 (memory) verdict "SIGNIFICANT ADVANTAGE" (4.0x vs INT8, matches README claim); Phase 3 started not finished; Phase 4-6 not run. Resume with `python3 -u benchmarks/python-with-interpreter-overhead/bench_competitive.py > <logfile> 2>&1 &` — always unbuffered to a real file, never piped through `tail` in a backgrounded shell (a prior attempt lost ~91 min of compute exactly that way)
