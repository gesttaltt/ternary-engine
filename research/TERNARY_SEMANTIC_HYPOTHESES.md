...
**Doc-Type:** Research Hypotheses · Version 1.1 · Created 2025-12-30

---

## Philosophy: Negative Space Discovery

**We do not search for solutions. We LISTEN to errors.**

By systematically falsifying hypotheses, we discover the solution space through its **negative image**. Each falsified hypothesis removes territory from the map of possibility. What remains after all eliminations IS the truth - revealed by negative force rather than explicit search.

**Principles:**
- Errors are not failures - they are **boundaries** showing where truth is NOT
- Remain skeptical BUT faithful - doubt claims, trust the process
- No preference ordering - each hypothesis deserves equal initial scrutiny
- Wire ALL components - test against real implementations, not abstractions

---

## Component Integration

All falsification tests MUST wire these project components:

| Component | Location | Purpose |
|-----------|----------|---------|
| **SIMD Engine** | `ternary_simd_engine.pyd` | Native ternary arithmetic (tadd, tmul, tmin, tmax, tnot) |
| **Hyperbolic VAE** | `models/3-vae-gemm-v1/hyperbolic_ops.py` | Poincaré ball embeddings, geodesic operations |
| **Ultrametric Energy** | `models/gemm_discovery/ebm/ultrametric_energy.py` | 3-adic valuation, ultrametric distance |
| **Codon Encoder** | `models/company-flagships/.../codon_encoder_3adic.pt` | Biological ultrametric hierarchy |
| **Operation LUTs** | `models/3-vae-gemm-v1/data.py` | Complete 19,683 operation corpus |
...
- **Errors are teachers** - each failure illuminates boundaries
